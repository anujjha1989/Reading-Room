...
async function staleWhileRevalidate(request, cacheName) {
  // Network-first, despite the name kept for the call site.
  //
  // This was stale-while-revalidate, which returned the cached catalogue at
  // once and refreshed behind it - so the library always rendered one visit
  // out of date. Harmless while book ids were stable; wrong once every id
  // changed, because a stale catalogue asks for covers by ids that no longer
  // exist and the app falls back to placeholder art.
  //
  // The catalogue carries an ETag, so a revalidation is a 304 and costs
  // almost nothing. The cache is kept strictly as the offline fallback.
  const cache = await caches.open(cacheName);
  try {
    const response = await fetch(request);
    if (response.ok) {
    ...
    throw new Error('catalogue fetch failed: ' + response.status);
  } catch (err) {
    const hit = await cache.match(request);
    if (hit) return hit;
    throw err;
