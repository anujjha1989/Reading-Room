...
const authorFixPath = join(dataDir, "author-fixes.json");
// Books whose Drive file is gone. catalog.json is rebuilt from static snapshots
// that never contacted Drive, so a deleted file reappears on every rescan -
// hiding it has to be recorded outside the catalogue, like the title fixes.
const hiddenIdsPath = join(dataDir, "hidden-ids.json");
...
let authorFixes = {};                // bookId -> corrected author
let hiddenIds = {};                  // bookId -> { at, reason }
...
async function loadCatalogAndCaches() {
  ...
  try { authorFixes = JSON.parse(await readFile(authorFixPath, "utf8")); } catch { authorFixes = {}; }
  try { hiddenIds = JSON.parse(await readFile(hiddenIdsPath, "utf8")); } catch { hiddenIds = {}; }
  try {
...
async function saveHiddenIds() {
  try {
    await mkdir(dataDir, { recursive: true });
    const tmp = join(dataDir, `.hidden-ids.${process.pid}.tmp`);
    await writeFile(tmp, JSON.stringify(hiddenIds));
    await rename(tmp, hiddenIdsPath);
  } catch (error) { console.error("Could not save hidden ids:", error.message); }
}

async function saveAuthorFixes() {
  try {
    await mkdir(dataDir, { recursive: true });
    const tmp = join(dataDir, `.author-fixes.${process.pid}.tmp`);
    await writeFile(tmp, JSON.stringify(authorFixes));
    await rename(tmp, authorFixPath);
  } catch (error) { console.error("Could not save author fixes:", error.message); }
}

// Hide a book whose file is gone. The client has been calling /api/quarantine
// since the long-press sheet existed, but the endpoint was never implemented -
// so Delete failed silently. It does not touch Drive: the file is already gone
// in the case this exists for, and the catalogue is a static snapshot that
// cannot learn that. Recording the id here removes it from every rebuild.
async function quarantine(request, response) {
  let body = "";
  for await (const chunk of request) {
    body += chunk;
    if (body.length > 8192) { response.writeHead(413).end("too large"); return; }
  }
  let payload;
  try { payload = JSON.parse(body || "{}"); } catch { response.writeHead(400).end("bad JSON"); return; }

  const id = String(payload.id || "");
  if (!id || !/^[A-Za-z0-9_-]{10,100}$/.test(id)) {
    sendJson(response, 400, { error: "bad id" });
    return;
  }
  // Deliberately not checked against the catalogue: a book can be hidden after
  // the catalogue has already dropped it, and re-hiding must stay idempotent.
  if (payload.restore) {
    delete hiddenIds[id];
  } else {
    hiddenIds[id] = { at: new Date().toISOString(), reason: String(payload.reason || "deleted") };
  }
  await saveHiddenIds();
  rebuildCatalog();
  sendJson(response, 200, { ok: true, hidden: Object.keys(hiddenIds).length });
}

// Record a correction made from the library. An empty string clears a field
// and lets the scanner's own value show through again.
async function metaFix(request, response) {
  let body = "";
  for await (const chunk of request) {
    body += chunk;
    if (body.length > 8192) { response.writeHead(413).end("too large"); return; }
  ...
  const id = String(payload.id || "");
  if (!id || !catalog.some((b) => b.id === id)) {
    response.writeHead(404, { "content-type": "application/json" })
      .end(JSON.stringify({ error: "unknown book" }));
    return;
  }

  const tidy = (v) => String(v == null ? "" : v).replace(/\s+/g, " ").trim().slice(0, 300);
  let changed = false;
  if (Object.prototype.hasOwnProperty.call(payload, "title")) {
    const t = tidy(payload.title);
    if (t) titleFixes[id] = t; else delete titleFixes[id];
    changed = true;
    await saveTitleFixes();
  }
  if (Object.prototype.hasOwnProperty.call(payload, "author")) {
    const a = tidy(payload.author);
    if (a) authorFixes[id] = a; else delete authorFixes[id];
    changed = true;
    await saveAuthorFixes();
  }
  if (changed) rebuildCatalog();
...
async function sendLocalCover(response, id) {
  ...
  response.writeHead(200, {
    ...
    // Was immutable for a year, on the reasoning that a Drive id addressed
    // fixed art. Ids are path-derived now and the art behind one can be
    // re-extracted or regenerated, so a year-long pin strands the old image
    // in every browser that saw it. A day, then revalidate: a 304 is cheap.
...
async function cover(response, url) {
  ...
  if (isFreshMiss(cached)) {
    response.writeHead(404, { "cache-control": "public, max-age=300" }).end("Cover unavailable");
    return;
  }
...
function statsPage() {
  ...
  const byId = new Map(catalog.map((b) => [b.id,
    { ...b, title: titleFixes[b.id] || b.title, author: authorFixes[b.id] || b.author }]));
  const label = (s) => {
  ...
  try {
    ...
    switch (url.pathname) {
      ...
      case "/api/meta-fix":
        ...
        await metaFix(request, response); return;
      case "/api/quarantine":
        if (request.method !== "POST") { response.writeHead(405).end("POST only"); return; }
        await quarantine(request, response); return;
      case "/api/health": {
